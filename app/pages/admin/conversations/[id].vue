...
<template>
  <div>
    <NuxtLink to="/admin/conversations" class="mb-4 inline-block text-sm text-primary-600 hover:underline">
      &larr; 一覧に戻る
    </NuxtLink>
    ...
    <template v-else-if="conversation">
      <div class="mb-6">
        ...
        <div class="mt-2 flex flex-wrap items-center gap-3 text-sm text-gray-500">
          ...
          <span v-if="conversation.userId === 'guest'" class="inline-flex items-center rounded-full bg-teal-100 px-2 py-0.5 text-xs font-medium text-teal-700">ウィジェット</span>
      ...
      <div class="space-y-4">
        ...
        >
          ...
          <!-- 参照元（開閉可能） -->
          <div v-if="msg.sources && msg.sources.length > 0" class="mt-3 border-t border-gray-100 pt-2">
            <button
            ...
            <div v-if="openSources.has(msg.id)" class="mt-2 space-y-2">
              ...
              >
                ...
                <div v-if="src.chunkContent" class="mt-1 text-gray-500">{{ src.chunkContent }}...</div>
                <div class="mt-1 text-gray-400">類似度: {{ (src.similarity * 100).toFixed(0) }}%</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="mt-6">
        ...
        <div v-if="feedbackOpen" class="mt-3 rounded-lg border border-gray-200 bg-white p-5">
          <div v-if="feedbackLoading" class="py-6 text-center text-sm text-gray-400">読み込み中...</div>
          <template v-else>
