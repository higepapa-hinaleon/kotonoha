...
const passwordMismatch = computed(
  () =>
    signupPasswordConfirm.value.length > 0 && signupPassword.value !== signupPasswordConfirm.value,
);
...
<template>
  <div class="rounded-lg bg-white p-6 shadow-md">
    ...
    <form class="space-y-4" @submit.prevent="handleEmailSignup">
      ...
      <div>
        <label for="signupPasswordConfirm" class="block text-sm font-medium text-gray-700">
        ...
        />
        <p v-if="passwordMismatch" class="mt-1 text-xs text-red-500">パスワードが一致しません</p>
      </div>
      <button
        type="submit"
    ...
    <p class="mt-4 text-center text-sm text-gray-500">
      既にアカウントをお持ちの方は
      <NuxtLink to="/login" class="text-primary-600 underline hover:text-primary-700"
        >こちら</NuxtLink
      >
    </p>
  </div>
